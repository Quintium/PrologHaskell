...
substStep (Left failure) _ = Left failure
substStep (Right (Subst vs f)) (t1, t2) = chainSubst (Subst vs f) (unify (f t1) (f t2)) 

-- Substitutions to be chained should have disjunct variables
chainSubst :: Subst -> Either UnifyFailure Subst -> Either UnifyFailure Subst
chainSubst _ (Left failure) = Left failure
chainSubst (Subst vs1 f1) (Right (Subst vs2 f2)) = Right (Subst (vs1 ++ vs2) (f2 . f1))
