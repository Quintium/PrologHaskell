...
functionP = FunctionP <$> (expressionP ",.():-\n" <* spaceP <* charP '(' <* spaceP) <*> sepBy (charP ',') termP <* charP ')'
...
processRule (RuleP head tails) = evalState (Rule <$> processTerm head <*> mapM processTerm tails) emptyVarNames

processQuery :: QueryP -> Query
processQuery (QueryP ts) = let (terms, vn) = runState (mapM processTerm ts) emptyVarNames
                           in Query terms vn